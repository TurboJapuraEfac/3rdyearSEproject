...
import { order_request , delivery_list } from '../../redux/ActionCreators';
...
class OrderRequests extends Component {
        ...
        firebase.firestore().collection('users').onSnapshot(snapshot => {
            snapshot.forEach(doc => {
                if (doc.data().isDelivery) {
                    ...
                    this.state.deliveryList.push(obj);
 ...
 async handleSendToInProgressBtn(userUid, orderId) {
        const { userDetails } = this.state; 
        const shopUid = userDetails.userUid
        const deliverPerson = this.state.selectedPerson
           
        const orderdetailsnew = await this.getOrder(shopUid,orderId);
        console.log("Orderdetails",orderdetailsnew)

        const newShopOrder ={
            ...orderdetailsnew,
            shopname : userDetails.userName,
            shopId : userDetails.userUid,
            status :"IN PROGRESS"
        }
        
        firebase.firestore().collection('users').doc(deliverPerson).collection('shopOrder').doc(orderId).set(
        newShopOrder)
        .then(() => {
            console.log("Succesfully created the ShopOrder")})
            .then(() => {
            firebase.firestore()
            .collection('users')
            .doc(shopUid)
            ...
            console.log("Sucessfully updated Shop Request")})})
            .then(() => {
            firebase.firestore()
            .collection('users')
            .doc(userUid)
            ...
            .update({
                status: "IN PROGRESS",
            }).then(() => {console.log("Sucessfully updated Cutomer Order")})
        }).catch(function(error) {
            console.log("Error getting document:", error);
        });
        console.log("selected person",deliverPerson)
    }
    ...
    handleSendToDeliveredBtn(userUid, orderId) {
        const { userDetails } = this.state;
        const shopUid = userDetails.userUid
        firebase.firestore().collection('users').doc(shopUid).collection('orderRequest').doc(orderId).update({
            status: "DELIVERED",
        }).then(() => {
                    ...
                    return (
                        <div className="container border-bottom pb-2 px-lg-0 px-md-0 mb-4" key={orderRequest[val].id}>
                                    ...
                                    return (
                                        <div className="row mb-3" key={val2}>
                                            <div className="col-lg-2 col-md-3 col-8 offset-2 offset-lg-0 offset-md-0 px-0 mb-3 text-center">
                                                <img style={{ width: "70px", height: "70px" }}  src={orderRequest[val].itemsList[val2].itemImageUrl} />
                                            </div>
                            ...
                            <div className="row mb-3 mb-md-0 mb-lg-0">
                                ...
                                <DropdownButton  id="dropdown-item-button" title="Select a delivery person" value={this.state.selectedPerson.id}  
                                onSelect={this.handleSelect}>
                                {this.state.deliveryList.map(person=>{
                                return (<Dropdown.Item  as="button" eventKey={person.id}>
                                <div className="row mb-3">  
                                    <div className="col-lg-7 col-md-6 col-sm-12 px-0">
                                    <h6 className="">{person.userName}</h6>
                                    <p className="mb-1"><small>TEL : {person.userMobile}</small></p>
                                     <p className="mb-1"><small>NIC : {person.userNIC}</small></p>
                                     <p className="mb-1"><small>LOC : {person.userCity}</small></p>
                                    </div>
                                </div>  
                                </Dropdown.Item>)})}
                                </DropdownButton>
                            ...
                            <div className="container">
                                {
                                    userDetails ? <div className="row">
                                        ...
                                        <div className="col-lg-10 col-md-9 col-12 pl-lg-0 pl-md-0">
                                            ...
                                            <p className="shop-text">{userDetails.typeOfGood && userDetails.typeOfGood.join(', ')}</p>
                                        </div>
                                    </div> : null
                                }
                            </div>
